...
def solve(P=p_matrix, I=I_matrix, O=O_matrix,I_T=I_T, O_T=O_T, e_T=e_T, max_time = 600):
    ...
    output_volume = [model.NewIntVar(0, 1000000, f"output_volume_{i}") for i in range(num_iv)]
    output_error =  [model.NewIntVar(0, 1000000, f"output_error_{i}") for i in range(num_iv)]
    squared_output_error = [model.NewIntVar(0, 10000000, f"squared_output_error_{i}") for i in range(num_iv)]
